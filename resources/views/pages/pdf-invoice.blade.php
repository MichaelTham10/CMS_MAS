  ...
  <div>
      ...
      <div class="info" style="">
        ...
        <div style="padding-left: 470px; width: 50%; margin-top: 15px;">
          ...
          <span>
            <strong>Quotation No</strong><span class="break-word" style="display: inline-block; margin-left: 37.5px;"></span>: {{$invoice->quotation['Quotation_No']}} <br>
          </span>
          <span>
            <strong>Quotation Date</strong><span class="break-word" style="display: inline-block; margin-left: 28.5px;"></span>: {{date("d/m/y", strtotime($invoice->quotation['Quotation Date']))}} <br>
          </span>
        </div>
      </div>  
        ...
        <tbody>
          @foreach ($invoice->items($invoice['Quotation No']) as $item)
            <tr>
              ...
              <td class="break-word"  style=""><strong>{{$item->name}}</strong></td>
      ...
      <div class="table-responsive">
        <div class="table-responsive m-10">
          <table class="table table-bordered no-margin table-sm" style="background-color: #c7c7c7">
            <tr>
              <th colspan="2" style="padding-left: 2%"  scope="row">Discount</th>
              <td  class="border-total break-word "style="font-weight: bold; width:15%;">IDR. {{number_format((($total / 100) * $invoice->quotation->Discount))}}.-</td>
            </tr>
            <tr>
            ...
            <tr>
              <th colspan="2" scope="row" style="padding-left: 2%;">Sub Total</th>
              <td class="border-total" style="border-bottom: 2px solid black; font-weight: bold; width:15%;">IDR. {{number_format(($total + $ppn))}}.-</td>
            </tr>
            @if($invoice->payment_status != "Down Payment")
              @foreach ($invoice_dps as $invoice_dp)
                <tr>
                  ...
                  <td  class="border-total break-word "style="font-weight: bold; width:15%;">IDR. {{number_format(($total + $ppn - (($total / 100) * $invoice->quotation->Discount)) * ($invoice_dp->dp_percent/100))}}.-</td>
                </tr>
                @php
            ...
            @else
              <tr>
                ...
                <td  class="border-total break-word "style="font-weight: bold; width:15%;">IDR. {{number_format(($total + $ppn - (($total / 100) * $invoice->quotation->Discount)) * ($invoice->dp_percent/100))}}.-</td>
              </tr>
              @php
            ...
            <tr>
              ...
              <td class="border-total" style="border-bottom: 2px solid black; font-weight: bold; width:15%;">IDR. {{number_format(($total + $ppn - (($total / 100) * $invoice->quotation->Discount)) - $dp)}}.-</td>
            </tr>
          </table>
        </div>
      </div>
